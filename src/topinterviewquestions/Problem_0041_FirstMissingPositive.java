 ...
 * 2 过程如下 arr[ 6,1,4,3,1,2,5,9]
 *                0 1 2 3 4 5 6 7
 * 开始时 L=0 R=9  arr[L]为6  要将6放到arr[5]的位置 交换后  arr[ 2,1,4,3,1,6,5,9]
 *                                                            0 1 2 3 4 5 6 7
 * 再次计算 L=0 R=9  arr[L]为2  要将2到arr[1]的位置       arr[ 1,2,4,3,1,6,5,9]
 *                                                           0 1 2 3 4 5 6 7
 * 此时arr[L=0]=1 L++ 再次计算  arr[L=1]=2 L++  L=2      arr[ 1,2,3,4,1,6,5,9]
 *                                                           0 1 2 3 4 5 6 7
 * 此时arr[L=2]=3 L++ L=3 再次计算  arr[L=3]=4 L++  L=4
 *
 * 此时arr[L=4]=1  1<=L  就是说 1~L+1上的值 我们已经有了  所以此时的1也是垃圾数据
 * 把arr[L=4] 交换到最后的垃圾区域  同时R-- L不动 R=6     arr[ 1,2,3,4,9,6,5,1]
 *                                                          0 1 2 3 4 5 6 7
 * 再次计算  arr[L=4]=9 > R+1 = 6+1 =7 我们最好的预期是  1234567 所以9也是垃圾数据
 * 把arr[L=4] 交换到最后的垃圾区域  同时R-- L不动 R=5    arr[ 1,2,3,4,5,6,9,1]
 *                                                         0 1 2 3 4 5 6 7
 * 此时arr[L=4]=5 L++ L=5  再次计算  arr[L=5]=6 L++  L=6  L=R 结束
 *
 *
 * 【面试技巧】
