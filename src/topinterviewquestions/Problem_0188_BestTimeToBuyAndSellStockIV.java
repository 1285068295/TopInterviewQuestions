...
public class Problem_0188_BestTimeToBuyAndSellStockIV {
    ...
    public static int maxProfit1(int K, int[] prices) {
        ...
        //     ...
        //    2.1)  dp[i][j] = dp[i][j-1]  + prices[i]-prices[i]   最后一次的交易为  最后一天买入 最后一天卖出(买入花钱-  卖出挣钱 +)
...
//        int tmp = 0;
        for (int j = 1; j <= K; j++) {
            // 踩坑点  tmp的初始值设定！！！  i=0 j=j-1
            int best = dp[0][j - 1] - prices[0];
            for (int i = 1; i < prices.length; i++) {
                //  斜率没有优化版本
                // [i]位置股票不参与交易
                dp[i][j] = dp[i-1][j];
                // [i-1]位置股票参与交易          买入-          卖出+                             best
//                                  dp[i-1][j-1]  - prices[i-1] + prices[i-1];          dp[i-1][j-1]  - prices[i-1];
//               dp[i-1][j] =max{   dp[i-2][j-1]  - prices[i-2] + prices[i-1]; } = max{ dp[i-2][j-1]  - prices[i-2];  } + prices[i-1] =  best + prices[i-1]
//                                  dp[i-3][j-1]  - prices[i-3] + prices[i-1];          dp[i-3][j-1]  - prices[i-3];
//                                  dp[i-4][j-1]  - prices[i-4] + prices[i-1];          dp[i-4][j-1]  - prices[i-4];
//               ...
//                                  dp[i][j-1]    - prices[i]   + prices[i];            dp[i][j-1]    - prices[i];
//                                  dp[i-1][j-1]  - prices[i-1] + prices[i];            dp[i-1][j-1]  - prices[i-1];
//               dp[i][j] = max{    dp[i-2][j-1]  - prices[i-2] + prices[i]; }   = max{ dp[i-2][j-1]  - prices[i-2]; } + prices[i]  =max(best,  dp[i][j-1] - prices[i]) + prices[i]
//                                  dp[i-3][j-1]  - prices[i-3] + prices[i];            dp[i-3][j-1]  - prices[i-3];
//                                  dp[i-4][j-1]  - prices[i-4] + prices[i];            dp[i-4][j-1]  - prices[i-4];
//               ...
...
//                }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(maxProfit1(2,arr));
